...
const handleChangeAvatar = (temp:string) => {
  setAvatarName(temp);
  setAvatar(temp);
};
  ...
  return (
    <InputArea>
      <Table>
        ...
        <Column2><Input type="text" maxLength={15}  onChange={handleChangeAuthor} value={author} /></Column2>
      ...
      <div>
        <Avatar onClick={() => handleChangeAvatar(ninja)} src={ninja} />
        <Avatar onClick={() => handleChangeAvatar(monkey)}src={monkey} />
        <Avatar onClick={() => handleChangeAvatar(pinguin)}src={pinguin} />
        <Avatar onClick={() => handleChangeAvatar(lamb)}src={lamb} />
        <Avatar onClick={() => handleChangeAvatar(stranger)}src={stranger}/>
      </div>
    </InputArea>
  );
}
